        ...
        if request.method == "POST":
            ...
            if noa == 1:
                ...
                if cans.answer == ans:
                    ...
                    if la.zone == "None" or la.zone == "Used" :
                        ms = ['2','1']
                    if  la.zone == "Red":
                        ms = ['4','2']
                    if la.zone == "Purple" :
            ...
            if cans.answer == ans or noa == 0:
                ...
                ul.save()
                score = la.score
                return render(request, 'quiz.html', {'title': "Quiz", 'q': q, 'noa': noa, 'score': score,'r1':resp1,'mins':mins,'secs':secs,'ms':texttolist(ul.markingscheme),'zone':zone,'zone_active':la.zone_activate,'lazone':la.zone,'riddle':la.riddle_activate,'uz':['None','Used','Red']})
            else:
            ...
            if response.objects.filter(user=request.user,queid=id).exists():
                ...
                resp1 = None
            zone = la.zone
            return render(request, 'quiz.html', {'title': "Quiz", 'q': q, 'noa': noofattempts, 'score': score,'r1':resp1,'mins':mins,'secs':secs,'ms':texttolist(ul.markingscheme),'zone':zone,'zone_active':la.zone_activate,'lazone':la.zone,'riddle':la.riddle_activate,'uz':['None','Used','Red']})
    else:
        messages.error(
...
def finish(request):
    if request.user.is_authenticated:
        ...
        score = la.score
        auth.logout(request)
        return render(request, 'result.html', {'title': 'Result', 'score': score, 'man': man})
    else:
        messages.error(
            request, 'You have already attempted quiz and Result was displayed at that time')
        return redirect('/')
...
def riddleque(request):
    if request.user.is_authenticated:
        ...
        if request.method == "POST":
            ...
            return render(request,"riddle.html",{'title':'Riddle','mins':mins,'secs':secs,'score': score,'ms':ms,'q':q})
    else:
        messages.error(
            request, 'You cannot start quiz because you have not "Login" yet.')
        return redirect('/')
        ...
        if request.user.is_staff:
            ...
            if request.method == "POST":
                ...
                if User.objects.filter(username=username).exists():
                    ...
                    messages.error(request,M)
            return render(request, 'superuser.html', {'title': 'Superuser panel','status':status,'Name':Name,'fname':fname,'Username':Username,'Email':Email,'Score':Score,'ul':ul,'al':al,'cl':cl,'login':login,'NRU':NRU,'NLU':NLU,'NCU':NCU,'RU':User.objects.all().exclude(is_staff = True),'LU':Profile.objects.all(),'exptlgt':exptlgt,'fstlgn':fstlgn,'comptime':comptime,'que':que,'lq':len(que),'timetaken':timetaken,'l1':l1,'l2':l2,'l1l2':l1 or l2})
        else:
            ul = userlist.objects.get(user=request.user)
            if ul.unattemptedlist != "":
                la = Profile.objects.get(user=request.user)
                ...
                auth.logout(request)
                messages.error(request, 'You have already attempted quiz')
                return redirect('/')
    else:
        messages.error(
